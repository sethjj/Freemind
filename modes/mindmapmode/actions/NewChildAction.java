...
//		         return null;
	...
	public MindMapNode addNewNode(MindMapNode parent, int index, boolean newNodeIsLeft){
        ...
        MindMapNode newChild = (MindMapNode) parent.getChildAt(index);
        /*
        String parentId = parent.getAttribute("id");
        newChild.createAttributeTableModel();
        
        NodeAttributeTableModel atm = newChild.getAttributes();
        int rowCount = atm.getRowCount();
        for( int i=rowCount-1; i>=0; i-- )
            atm.removeRow(i);
        atm.insertRow(0, "id", DataBaseReader.getTempId());
        atm.insertRow(1, "parentId", parentId);
