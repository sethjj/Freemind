...
public class FreeMind extends JFrame implements FreeMindMain {
	...
	public static final VersionInformation VERSION = new VersionInformation("0.9.0");
					...
					for (int i = 0; i < mTabbedPaneMapModules.size(); ++i) {
						if (mTabbedPaneMapModules.get(i) ==	pMapModule) {
							mTabbedPane.setTitleAt(i, pNewMapTitle + ((pModel.isSaved())?"":"*"));
                                                        //TODO JWW added to communicate title
                                                        System.out.println("Setting title to "+pNewMapTitle);
                                                        ((MindMapController)pMapModule.getModeController()).getMap().setMapName(pNewMapTitle);
						}
					}
				}
			});
...
//			}
	 ...
	 public ModeController createModeController(final String[] args) {
	...
	public void loadMaps(final String[] args, ModeController pModeController) {
		boolean fileLoaded = false;
                Logger.getLogger(getClass().getName()).log(Level.INFO, pModeController.getClass().getSimpleName());
