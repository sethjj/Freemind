...
public class PasteAction extends AbstractAction implements ActorXml {
    ...
    public PasteNodeAction getPasteNodeAction(Transferable t, NodeCoordinate coord) {
        ...
        String targetStr = pMindMapController.getNodeID(coord.target);
        Object tContent = getTransferableContent(t);
    ...
    public void paste(MindMapNode node, MindMapNode parent) {
        if (node != null) {
            ...
            node.setAttribute(0, new Attribute("parentId", parentId));
    ...
    private class MindMapNodesFlavorHandler implements DataFlavorHandler {
        ...
        public void paste(Object TransferData, MindMapNode target, boolean asSibling, boolean isLeft, Transferable t) {
            ...
            if (textFromClipboard != null) {
                ...
                MindMap mm = pMindMapController.getMap();
                MindMapNode root = mm.getRootNode();
                ...
                try {
                    ...
                    try {
                        ...
                        if (referenceURL == null && !baseUrlCanceled) {
                            String referenceURLString = JOptionPane.showInputDialog(
    ...
    public MindMapNodeModel pasteXMLWithoutRedisplay(String pasted,
        ...
        DiffAction da = new DiffAction(pMindMapController);
        try {
            MindMapNodeModel node = (MindMapNodeModel) pMindMapController.createNodeTreeFromXml(new StringReader(pasted), pIDToTarget);
            // try renumbering nodes here
            String idStr = node.getAttribute("id");
            if (idStr != null) {
                node = da.renumberNodes(node, target);
             } 
            // end of renumbering effort
    ...
    private void insertNodeInto(MindMapNode node, MindMapNode parent) {
        ...
        String parentId = parent.getAttribute("id");
        int parentIdPos = node.getAttributePosition("parentId");
        node.setAttribute(parentIdPos, new Attribute("parentId", parentId));
    }
